...
- Базовая тема — тёмная (ориентир на глубокие оттенки синего и графита с акцентами `accent-gold`), оформление включается через `dark` класс на `<html>` и системные преференции (`media` strategy в Tailwind).
